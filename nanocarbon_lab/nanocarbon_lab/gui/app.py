"""Tkinter desktop application for nanocarbon_lab.
...
Run with ``nanocarbon-gui`` or ``python -m nanocarbon_lab.gui``.
"""
...
Tkinter:
  ...
  nanocarbon cnt --n 6 --m 6 --length 10 --out salida --format both
""".strip()

_EXPORT_FORMATS = (
    ("qe", "Quantum ESPRESSO (pw.in)"),
...
class NanocarbonApp:
    ...
    def __init__(self, root) -> None:
        ...
        self.ttk = ttk
        self.root = root
        self.root.title("nanocarbon_lab — generador de nanoestructuras de carbono")
    ...
    def _rebuild_param_fields(self) -> None:
        ...
        self.calculation_var = self.tk.StringVar(value="scf")
        ttk.Label(fmts, text="Tipo de cálculo (QE)").pack(anchor="w", pady=(4, 0))
        ttk.Combobox(
            fmts, textvariable=self.calculation_var,
            values=["scf", "relax", "vc-relax", "nscf", "bands"],
            state="readonly",
        ).pack(fill="x")
    ...
    def _on_built(self, atoms: Atoms) -> None:
        ...
        self._render(atoms)
        self._set_info(describe_structure(atoms))
